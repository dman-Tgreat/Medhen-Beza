...
export function AdminTopbar({ onToggleMobileSidebar }: AdminTopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-border bg-surface px-4 md:px-6 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          ...
          <h1 className="text-base md:text-lg font-semibold text-text tracking-tight">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className="flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-1.5 text-xs font-medium text-text hover:bg-primary-light/50 transition-colors shadow-none focus:outline-none focus:ring-2 focus:ring-primary"
              title="Switch simulated user role to test permissions"
            >
              <div
                className={`flex h-6 w-6 items-center justify-center rounded-md border text-xs ${currentRoleOption.colorClass}`}
              >
        ...
        <DropdownMenu open={notificationsOpen} onOpenChange={setNotificationsOpen}>
          <DropdownMenuTrigger asChild>
            <button
              className="relative flex h-9 w-9 items-center justify-center rounded-md border border-border bg-surface text-text hover:bg-background transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
              aria-label="View notifications"
            >
              <Bell className="h-4 w-4" />
              <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-emergency text-[10px] font-bold text-white">
                3
              </span>
            </button>
          </DropdownMenuTrigger>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-full border border-border p-0.5 focus:outline-none focus:ring-2 focus:ring-primary">
              <img
