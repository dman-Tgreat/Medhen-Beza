...
export async function generateMetadata({
  ...
  const [service, settings] = await Promise.all([
    getPublicServiceBySlug(slug, locale),
...
export default async function ServiceDetailPage({ params }: ServicePageProps) {
  ...
  const [department, allDoctors, settings] = await Promise.all([
    getPublicDepartmentBySlug(service.departmentSlug, locale),
    ...
    <div className="min-h-screen bg-background pb-20">
      <JsonLd
        data={{
          ...
          provider: hospitalReference(settings.hospitalName),
        }}
      />
      <PageHero
        title={service.name}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="bg-surface rounded-2xl border border-border p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <h2 className="text-h3 font-bold text-text">
                {dict.services?.overview || "Service Overview"}
              </h2>
              ...
              {service.features && service.features.length > 0 && (
                <div className="border-t border-border pt-6 space-y-3">
                  <h3 className="text-h4 font-bold text-text">
                    {dict.services?.highlights || "Key Care Highlights"}
                  </h3>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-h3 font-bold text-text">
                    {dict.services?.relatedSpecialists || "Specialists Providing this Service"}
                  </h3>
                  <Link
                    href={`/${locale}/doctors`}
          ...
          <div className="lg:col-span-4">
            <div className="bg-surface rounded-2xl border border-border p-6 shadow-sm sticky top-24 space-y-6">
              <h3 className="text-h4 font-bold text-text">
                {dict.common?.details || "Service Information"}
              </h3>
              ...
              <div className="space-y-4 text-small">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-text block">
                      {dict.services?.availability || "Availability"}
                    </span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-text block">
                      {dict.services?.department || "Host Department"}
                    </span>
                    <Link
                      href={`/${locale}/departments/${service.departmentSlug}`}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-text block">
                      {dict.common?.phone || "Inquiry Phone"}
                    </span>
                    <a
              ...
              <div className="border-t border-border pt-4 space-y-2">
                <Button asChild variant="primary" className="w-full">
                  <Link href={`/${locale}/contact`}>
                    {dict.nav?.contact || "Book Service Appointment"}
                  </Link>
                </Button>
                <Button asChild variant="outline" className="w-full">
                  <Link href={`/${locale}/emergency`}>
                    {dict.common?.emergency247 || "Emergency Response (24/7)"}
                  </Link>
                </Button>
              </div>
            </div>
