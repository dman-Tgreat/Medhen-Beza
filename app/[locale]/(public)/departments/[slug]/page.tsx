...
export async function generateMetadata({
  ...
  const [dept, settings] = await Promise.all([
    getPublicDepartmentBySlug(slug, locale),
  ...
  return contentMetadata({
    ...
    path: `/${locale}/departments/${dept.slug}`,
...
export default async function DepartmentDetailPage({
  ...
  const [dept, settings, dict] = await Promise.all([
    getPublicDepartmentBySlug(slug, locale),
    getPublicSiteSettings(locale),
    ...
    <div className="min-h-screen bg-background pb-20">
      <JsonLd
        data={{
          ...
          url: absoluteUrl(`/${locale}/departments/${dept.slug}`),
          image: dept.image ? absoluteUrl(dept.image) : undefined,
          medicalSpecialty: dept.name,
          parentOrganization: hospitalReference(settings.hospitalName),
        }}
      />
      <PageHero
        title={`${dept.name}`}
        description={dept.description}
        badge={dict.nav?.departments || "Clinical Department"}
        breadcrumbs={[
          { label: dict.nav?.departments || "Departments", href: `/${locale}/departments` },
          { label: dept.name },
        ]}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-10">
            ...
            <div className="bg-surface rounded-2xl border border-border p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <h2 className="text-h3 font-bold text-text">
                {dict.services?.overview || "Department Overview"}
              </h2>
              ...
              {dept.keyServices && dept.keyServices.length > 0 && (
                <div className="border-t border-border pt-6 space-y-3">
                  <h3 className="text-h4 font-bold text-text">
                    {dict.services?.highlights || "Core Specializations"}
                  </h3>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-h3 font-bold text-text">
                    {dept.name} {dict.nav?.services || "Services"}
                  </h3>
                  <Link
            ...
            {departmentDoctors.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-h3 font-bold text-text">
                    {dept.name} {dict.nav?.doctors || "Specialists"}
                  </h3>
                  <Link
                    href={`/${locale}/doctors`}
                    className="text-small font-semibold text-primary hover:underline flex items-center gap-1"
          ...
          <div className="lg:col-span-4">
            <div className="bg-surface rounded-2xl border border-border p-6 shadow-sm sticky top-24 space-y-6">
              <h3 className="text-h4 font-bold text-text">
                {dict.common?.details || "Unit Information"}
              </h3>
              ...
              <div className="space-y-4 text-small">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-text block">
                      {dict.common?.hours || "Working Hours"}
                    </span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-text block">
                      {dict.departments?.lead || "Department Lead"}
                    </span>
              ...
              <div className="border-t border-border pt-4 space-y-2">
                <Button asChild variant="primary" className="w-full">
                  <Link href={`/${locale}/contact`}>
                    {dict.nav?.contact || "Inquire with Department"}
                  </Link>
                </Button>
                <Button asChild variant="outline" className="w-full">
                  <Link href={`tel:${dept.phone}`}>
                    {dict.common?.callNow || "Call Department"}
                  </Link>
                </Button>
              </div>
            </div>
